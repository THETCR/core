...
BOOST_AUTO_TEST_CASE(getlocator_test)
{
    ...
    for (unsigned int i=0; i<vBlocksMain.size(); i++) {
        ...
        BOOST_CHECK_EQUAL((int)uint256(vBlocksMain[i].GetBlockHash()).GetLow64(), vBlocksMain[i].nHeight);
    ...
    for (unsigned int i=0; i<vBlocksSide.size(); i++) {
        ...
        BOOST_CHECK_EQUAL((int)uint256(vBlocksSide[i].GetBlockHash()).GetLow64(), vBlocksSide[i].nHeight);
    ...
    for (int n=0; n<100; n++) {
        ...
        for (unsigned int i = 1; i < 12 && i < locator.vHave.size() - 1; i++) {
            BOOST_CHECK_EQUAL(uint256(locator.vHave[i]).GetLow64(), tip->nHeight - i);
        }

        // The further ones (excluding the last one) go back with exponential steps.
        unsigned int dist = 2;
        for (unsigned int i = 12; i < locator.vHave.size() - 1; i++) {
            BOOST_CHECK_EQUAL(uint256(locator.vHave[i - 1]).GetLow64() - uint256(locator.vHave[i]).GetLow64(), dist);
            dist *= 2;
        }
    }
}
...
BOOST_AUTO_TEST_CASE(findearliestatleast_test)
{
    ...
    std::vector<CBlockIndex> vBlocksMain(100000);
    for (unsigned int i=0; i<vBlocksMain.size(); i++) {
        vHashMain[i] = uint256(i); // Set the hash equal to the height
        vBlocksMain[i].nHeight = i;
        vBlocksMain[i].pprev = i ? &vBlocksMain[i - 1] : nullptr;
        vBlocksMain[i].phashBlock = &vHashMain[i];
        vBlocksMain[i].BuildSkip();
