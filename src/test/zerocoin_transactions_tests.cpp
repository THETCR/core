...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zwspTracker = unique_ptr<CzWSPTracker>(new CzWSPTracker(cWallet.strWalletFile, cWallet.chain(), cWallet.GetLocation(), WalletDatabase::Create(cWallet.GetLocation().GetPath())));
