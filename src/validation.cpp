...
//        CTxMemPoolEntry entry(&tx, nFees, GetTime(), dPriority, chainActive.Height());
        ...
        if (mapObfuscationBroadcastTxes.count(hash)) {
            ...
            } else if (!tx.IsZerocoinSpend() && gArgs.GetBoolArg("-relaypriority", true) && nFees < ::minRelayTxFee.GetFee(nSize)) {
            ...
            if (fLimitFree && nFees < ::minRelayTxFee.GetFee(nSize) && !tx.IsZerocoinSpend()) {
                ...
                if (dFreeCount >= gArgs.GetArg("-limitfreerelay", 30) * 10 * 1000)
...
//        CTxMemPoolEntry entry(tx, nFees, GetTime(), dPriority, chainActive.Height(), mempool.HasNoInputsOf(tx));
        ...
        if (isDSTX) {
            ...
            // Require that free transactions have sufficient priority to be mined in the next block.
            if (gArgs.GetBoolArg("-relaypriority", true) && nFees < ::minRelayTxFee.GetFee(nSize)) {
//                && !AllowFree(view.GetPriority(tx, chainActive.Height() + 1))
                return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "insufficient priority");
            }
...
bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    std::cout << "ReadBlockFromDisk finished\n";
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    //LogPrintf("Adjusting seesaw at height %d with %d masternodes (without drift: %d) at %ld\n", nHeight, nMasternodeCount, nMasternodeCount - Params().MasternodeCountDrift(), GetTime());
     ...
     LogPrint(BCLog::MASTERNODE, "GetMasternodePayment(): moneysupply=%s, nodecoins=%s \n", FormatMoney(nMoneySupply).c_str(),
    ...
    if (!tx.IsCoinBase() && !tx.IsZerocoinSpend()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (coin.IsCoinBase() || coin.IsCoinStake()) {
                if (nSpendHeight - coin.nHeight < Params().COINBASE_MATURITY())
                    return state.Invalid(
                            error("CheckInputs() : tried to spend coinbase at depth %d, coinstake=%d", nSpendHeight - coin.nHeight, coin.IsCoinStake()),
        ...
        if (tx.ContainsZerocoins()) {
                ...
                for (const CTxIn& txin : tx.vin) {
                    if (txin.scriptSig.IsZerocoinSpend()) {
                        ...
                        if (!zerocoinDB->EraseCoinSpend(spend.getCoinSerialNumber())){
                            error("failed to erase spent zerocoin in block");
                            return DISCONNECT_FAILED;
                        }
            ...
            if (tx.IsZerocoinMint()) {
                ...
                for (const CTxOut& txout : tx.vout) {
                    ...
                    PublicCoin pubCoin(Params().Zerocoin_Params(false));
                    if (!TxOutToPublicCoin(txout, pubCoin, state)){
                        error("DisconnectBlock(): TxOutToPublicCoin() failed");
                        return DISCONNECT_FAILED;
                    }

                    if(!zerocoinDB->EraseCoinMint(pubCoin.getValue())){
                        error("DisconnectBlock(): Failed to erase coin mint");
                        return DISCONNECT_FAILED;
                    }
                }
            }
    ...
    if (!fVerifyingBlocks) {
        ...
        if(nCheckpoint != pindex->pprev->nAccumulatorCheckpoint) {
            if(!EraseAccumulatorValues(nCheckpoint, pindex->pprev->nAccumulatorCheckpoint)){
                error("DisconnectBlock(): failed to erase checkpoint");
                return DISCONNECT_FAILED;
            }
        }
    }
...
void AddWrappedSerialsInflation()
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Adding Wrapped Serials supply..."), percent, true);
        }
...
void RecalculateZWSPMinted()
{
    ...
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = (int)( (double)(pindex->nHeight - Params().NEW_PROTOCOLS_STARTHEIGHT()) * 100 / (chainActive.Height() - Params().NEW_PROTOCOLS_STARTHEIGHT()) );
            uiInterface.ShowProgress(_("Recalculating minted ZWSP..."), percent, true);
        }

        //overwrite possibly wrong vMintsInBlock data
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex, consensusParams));
...
void RecalculateZWSPSpent()
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = (int)( (double)(pindex->nHeight - Params().NEW_PROTOCOLS_STARTHEIGHT()) * 100 / (chainActive.Height() - Params().NEW_PROTOCOLS_STARTHEIGHT()) );
            uiInterface.ShowProgress(_("Recalculating spent ZPIV..."), percent, true);
        }
...
bool RecalculateWSPSupply(int nHeightStart)
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating WSP supply..."), percent, true);
        }
...
bool ReindexAccumulators(std::list<uint256>& listMissingCheckpoints, std::string& strError)
{
    ...
    if (!listMissingCheckpoints.empty()) {
        uiInterface.ShowProgress(_("Calculating missing accumulators..."), 0, true);
        ...
        while (pindex) {
            uiInterface.ShowProgress(_("Calculating missing accumulators..."), std::max(1, std::min(99, (int)((double)(pindex->nHeight - nZerocoinStart) / (double)(chainActive.Height() - nZerocoinStart) * 100))), true);
...
//            REJECT_INVALID, "PoS-early");
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        } else if (!tx.IsCoinBase())
        {
            ...
            PrecomputedTransactionData _txdata(tx);
            unsigned int flags = SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG;
            if (fCLTVHasMajority)
                flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;

            if (!CheckInputs(tx, state, view, fScriptChecks, flags, false, false, _txdata, nScriptCheckThreads ? &vChecks : NULL))
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush && !pcoinsTip->GetBestBlock().IsNull()) {
            ...
            if (!CheckDiskSpace(GetDataDir(), 100 * 2 * 2 * pcoinsTip->GetCacheSize())) {
                return AbortNode(state, "Disk space is low!", _("Error: Disk space is low!"));
            }
...
//            if (**it.IsCoinBase() || **it.IsCoinStake() || !AcceptToMemoryPool(mempool, stateDummy, **it, false, NULL)){
            mempool.removeRecursive(**it, MemPoolRemovalReason::REORG);
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    {
        CCoinsViewCache view(pcoinsTip.get());
        std::cout << "ConnectTip ConnectBlock\n";
        bool rv = ConnectBlock(blockConnecting, state, pindexNew, view, chainparams);
        std::cout << "ConnectTip BlockChecked\n";
    ...
    // Write the chain state to disk, if necessary.
    std::cout << "ConnectTip FlushStateToDisk\n";
    if (!FlushStateToDisk(chainparams, state, FlushStateMode::IF_NEEDED))
        return false;
    int64_t nTime5 = GetTimeMicros(); nTimeChainState += nTime5 - nTime4;
    LogPrint(BCLog::BENCH, "  - Writing chainstate: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime5 - nTime4) * MILLI, nTimeChainState * MICRO, nTimeChainState * MILLI / nBlocksTotal);
    // Remove conflicting transactions from the mempool.;
    std::cout << "ConnectTip removeForBlock\n";
    mempool.removeForBlock(blockConnecting.vtx, pindexNew->nHeight);
    std::cout << "ConnectTip disconnectpool.removeForBlock\n";
    disconnectpool.removeForBlock(blockConnecting.vtx);
    // Update chainActive & related variables.
    chainActive.SetTip(pindexNew);
    std::cout << "ConnectTip UpdateTip\n";
...
//    std::list<CTransaction> txConflicted;
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0 && !foundConflictingTx && i < 6; i++) {
        ...
        CBlock block;
        if (!ReadBlockFromDisk(block, BlockReading, consensusParams))
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        for (CBlockIndex *pindexConnect : reverse_iterate(vpindexToConnect)) {
            if (!ConnectTip(state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(), connectTrace, disconnectpool)) {
                if (state.IsInvalid()) {
                    std::cout << "ActivateBestChainStep state invalid 1\n";
                    ...
                    std::cout << "ActivateBestChainStep UpdateMempoolForReorg 1\n";
                    UpdateMempoolForReorg(disconnectpool, false);
                    return false;
                }
            } else {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (pindexPrev != nullptr) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            LogPrint(BCLog::MNPAYMENTS, "CheckBlock(): Masternode payment check skipped on sync - skipping IsBlockPayeeValid()\n");
        }
    }
...
bool IsTransactionInChain(const uint256& txId, int& nHeightTx)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    CTransactionRef tx;
    return IsTransactionInChain(txId, nHeightTx, tx, consensusParams);
}
