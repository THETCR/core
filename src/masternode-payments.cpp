...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    const CTransactionRef& ptx = (nBlockHeight > Params().LAST_POW_BLOCK() ? block.vtx[1] : block.vtx[0]);
    const CTransaction& txNew = *ptx;
...
std::string CMasternodeBlockPayees::GetRequiredPaymentsString()
{
    ...
    for (CMasternodePayee& payee: vecPayments) {
        ...
        if (ret != "Unknown") {
            ret += ", " + address2 + ":" + std::to_string(payee.nVotes);
        } else {
            ret = address2 + ":" + std::to_string(payee.nVotes);
        }
    }
