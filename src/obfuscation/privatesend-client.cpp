...
//        LogPrint("privatesend", "CPrivateSendClient::CheckAutomaticBackup -- Very low number of keys left: %d, no mixing available.\n", pwalletMain->nKeysLeftSinceAutoBackup);
            strAutoDenomResult = strprintf(_("Very low number of keys left: %d") + ", " + _("no mixing available."),
...
//        LogPrint("privatesend", "CPrivateSendClient::CheckAutomaticBackup -- Very low number of keys left: %d\n", pwalletMain->nKeysLeftSinceAutoBackup);
            strAutoDenomResult = strprintf(_("Very low number of keys left: %d"),
                                           pwallet->nKeysLeftSinceAutoBackup);
...
//            LogPrint("privatesend", "CPrivateSendClient::CheckAutomaticBackup -- Trying to create new backup.\n");
                ...
                CWallet* pwalletBackup = pwallet.get();
                if (!AutoBackupWallet(pwalletBackup, "", warningString, errorString)) {
                    ...
                    if (!errorString.empty()) {
                        ...
                        LogPrintf(
                                ...
                                errorString);
                        strAutoDenomResult = strprintf(_("ERROR! Failed to create automatic backup") + ": %s",
                                                       errorString);
                        return false;
                    }
                }
...
bool CPrivateSendClient::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated, CConnman& connman)
{
    ...
    const auto locked_chain = pwalletMain->chain().lock();
    bool fSuccess = pwalletMain->CreateTransaction(*locked_chain, vecSend, wtx, reservekeyChange,
...
//        LogPrintf("CPrivateSendClient::MakeCollateralAmounts -- ONLY_NONDENOMINATED: %s\n", strFail);
        ...
        if(fTryDenominated) {
            ...
            if(!pwalletMain->CreateTransaction(*locked_chain, vecSend, wtx, reservekeyChange,
