        ...
        || it->type == MSG_MASTERNODE_ANNOUNCE || it->type == MSG_MASTERNODE_PING || it->type == MSG_MASTERNODE_WINNER || it->type == MSG_DSTX)) {
            ...
            it++;
             ...
             if (inv.IsKnownType()) {
                ...
                if (!push && inv.type == MSG_BUDGET_VOTE) {
                    if (budget.mapSeenMasternodeBudgetVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                ...
                if (!push && inv.type == MSG_BUDGET_PROPOSAL) {
                    if (budget.mapSeenMasternodeBudgetProposals.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                ...
                if (!push && inv.type == MSG_BUDGET_FINALIZED_VOTE) {
                    if (budget.mapSeenFinalizedBudgetVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                ...
                if (!push && inv.type == MSG_BUDGET_FINALIZED) {
                    if (budget.mapSeenFinalizedBudgets.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::GETBLOCKS) {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            ...
            const int nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / chainparams.GetConsensus().nTargetSpacingV2;
    ...
    if (strCommand == NetMsgType::GETHEADERS && Params().HeadersFirstSyncingActive()) {
        CBlockLocator locator;
        uint256 hashStop;
        vRecv >> locator >> hashStop;
    ...
    if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX) {
        ...
        if (strCommand == NetMsgType::DSTX){
            ...
            if (pmn != nullptr) {
                if (!pmn->allowFreeTx) {
                    ...
                    LogPrint(BCLog::MASTERNODE, "dstx: Masternode sending too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
        ...
        } else if (tx.IsZerocoinSpend() && AcceptToMemoryPool(mempool, state, ptx, &fMissingZerocoinInputs, &lRemovedTxn, ignoreFees, 0)) {
    ...
    if (strCommand == NetMsgType::HEADERS && Params().HeadersFirstSyncingActive())
    {
...
//        }
    ...
    if (strCommand == NetMsgType::ACCVALUE) {
        if(pfrom->GetLocalServices() & NODE_BLOOM_LIGHT_ZC) {
            try {
                int height;
    ...
    if (strCommand == NetMsgType::GENWIT) {
        if(pfrom->GetLocalServices() & NODE_BLOOM_LIGHT_ZC) {
            try {
                CGenWit gen;
                vRecv >> gen;
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                state.nHeadersSyncTimeout = GetTimeMicros() + HEADERS_DOWNLOAD_TIMEOUT_BASE + HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (GetAdjustedTime() - pindexBestHeader->GetBlockTime())/(consensusParams.nTargetSpacingV2);
                nSyncStarted++;
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (nNow > state.nDownloadingSince + consensusParams.nTargetSpacingV2 * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
