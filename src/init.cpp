...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-rpccorsdomain=<domain>", "Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the Wispr GUI in a browser.", false, OptionsCategory::RPC);
...
#endif
    using PSETPROCDEPPOL = BOOL (*)(DWORD);
...
bool AppInitParameterInteraction()
{
    ...
    if (!fWisprMode)
    {
        ...
        if (gArgs.GetBoolArg("-regtest", false))
            ResetParams(CBaseChainParams::REGTEST, fWisprMode);
    };
    ...
    if ((!fEnableReplacement) && gArgs.IsArgSet("-mempoolreplacement")) {
        ...
        Split(vstrReplacementModes, strReplacementModeList, ",");
...
#ifdef ENABLE_WALLET
    if (fWisprMode) // SMSG breaks functional tests with services flag, see version msg
    {
