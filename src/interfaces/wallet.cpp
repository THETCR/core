...
public:
        ...
        std::string& reject_reason, std::string strCommand) override
    {
        ...
        if (!m_wallet.CommitTransaction(m_tx, std::move(value_map), std::move(order_form), m_key, state, strCommand)) {
...
public:
    ...
    bool haveMultiSig() override { return m_wallet->HaveMultiSig(); };
                                                       ...
                                                       bool useIX = false) override
    {
        ...
        if (!m_wallet->CreateTransaction(*locked_chain, recipients, pending->m_tx, pending->m_key, fee, change_pos,
                fail_reason, coin_control, sign, coin_type, useIX)) {
            return {};
        }
    ...
    WalletBalances getBalances() override
    {
        ...
        if(m_wallet->HasZerocoinFeatures()){
            result.zerocoin_balance = bal.m_zerocoin_trusted;
            result.unconfirmed_zerocoin_balance = bal.m_zerocoin_untrusted_pending;
            result.immature_zerocoin_balance = bal.m_zerocoin_immature;
        }
    ...
    CAmount getLockedWatchOnlyBalance() override
    {
        return m_wallet->GetLockedWatchOnlyBalance();
    }
