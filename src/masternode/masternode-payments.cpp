...
//    LogPrint("gobject", "block.vtx[0]->GetValueOut() %lld <= nSuperblockMaxValue %lld\n", block.vtx[0]->GetValueOut(), nSuperblockMaxValue);
    if(nBlockHeight == Params().NEW_PROTOCOLS_STARTHEIGHT()){
        return true;
    }
...
std::string CMasternodeBlockPayees::GetRequiredPaymentsString() const
{
    ...
    for (const auto& payee : vecPayees)
    {
        ...
        strRequiredPayments += strprintf("%s:%d", address2, payee.GetVoteCount());
    }
...
//            CBitcoinAddress address2(address1);
            ...
            debugStr += strprintf("    - %s - voted for %s\n",
                                  mn.second.outpoint.ToStringShort(), address2);
        } else {
...
////                CBitcoinAddress address2(address1);
//                std::string address2 = EncodeDestination(address1);
//                printf("payee %s votes %d\n", address2.c_str(), payee.GetVoteCount());
//            }
