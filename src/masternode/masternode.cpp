...
//                LogPrint("masternode", "CMasternode::Check -- Masternode %s is in %s state now\n", outpoint.ToStringShort(), GetStateString());
        ...
        // part 1: expire based on bitcoind ping
...
//    int mainnetDefaultPort = SelectParams(CBaseChainParams::MAIN).GetDefaultPort();
...
//        LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 bitcoin, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
...
bool CMasternodeBroadcast::Sign(const CKey& keyCollateralAddress)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        ...
        std::string strMessage = addr.ToString() + strprintf("%d", sigTime) +
                        pubKeyCollateralAddress.GetID().ToString() + pubKeyMasternode.GetID().ToString() +
                        strprintf("%d", nProtocolVersion);
