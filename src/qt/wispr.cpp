...
public:
    ...
    /// Get window identifier of QMainWindow (WISPRGUI)
...
{
    window = new WISPRGUI(networkStyle, nullptr);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(WISPRGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(WISPRGUI::DEFAULT_WALLET);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", WISPRGUI::tr("A fatal error occurred. WISPR can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(nullptr, QObject::tr("WISPR Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
