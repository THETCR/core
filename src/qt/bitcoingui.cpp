    ...
    m_network_style(networkStyle)
{
    ...
    labelWalletEncryptionIcon->setStyleSheet(".QPushButton { background-color: rgba(255, 255, 255, 0);}");
    labelWalletEncryptionIcon->setMaximumSize(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE);
...
void BitcoinGUI::createActions()
{
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible WISPR command-line options").arg(tr(PACKAGE_NAME)));
    ...
    if(walletFrame)
    {
        connect(labelWalletEncryptionIcon, &GUIUtil::ClickableLabel::clicked, walletFrame, &WalletFrame::toggleLockWallet);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to WISPR network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::updateHeadersSyncProgressLabel()
{
    ...
    int estHeadersLeft = (GetTime() - headersTipTime) / Params().GetConsensus().nTargetSpacingV2;
...
void BitcoinGUI::setAutoMintStatus()
{
    if (walletFrame) {
        if (fEnableZeromint) {
            ...
            labelAutoMintIcon->setPixmap(platformStyle->SingleColorIcon(":/icons/automint_active").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
            labelAutoMintIcon->setToolTip(
                    tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n");
        } else {
            labelAutoMintIcon->show();
            labelAutoMintIcon->setPixmap(platformStyle->SingleColorIcon(":/icons/automint_inactive").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
...
void BitcoinGUI::updateProxyIcon()
{
    ...
    if (proxy_enabled) {
        if (labelProxyIcon->pixmap() == nullptr) {
            ...
            labelProxyIcon->setPixmap(platformStyle->SingleColorIcon(":/icons/onion").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
