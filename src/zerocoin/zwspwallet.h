...
public:
    ...
    void SyncWithChain(CHDWallet *pwallet, bool fGenerateMintPool = true);
    ...
    void GenerateMintPool(CHDWallet *pwallet, uint32_t nCountStart = 0, uint32_t nCountEnd = 0);
    bool LoadMintPoolFromDB(CHDWallet *pwallet);
    void RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes);
    bool SetMintSeen(CHDWallet *pwallet, const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    bool IsInMintPool(const CBigNum& bnValue) { return mintPool.Has(bnValue); }
    void UpdateCount(CHDWallet *pwallet);
    void Lock();
