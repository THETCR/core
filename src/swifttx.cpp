...
bool IsIXTXValid(const CTransaction& txCollateral)
{
    ...
    if (nValueOut > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
        LogPrint(BCLog::SWIFTX, "IsIXTXValid - Transaction value too high - %s\n", txCollateral.ToString().c_str());
        return false;
    }

    if (missingTx) {
        LogPrint(BCLog::SWIFTX, "IsIXTXValid - Unknown inputs in IX transaction - %s\n", txCollateral.ToString().c_str());
        /*
    ...
    if (nValueIn - nValueOut < COIN * 0.01) {
        LogPrint(BCLog::SWIFTX, "IsIXTXValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString().c_str());
        return false;
    }
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())) {
        ...
        LogPrint(BCLog::SWIFTX, "CreateNewLock - Transaction Lock Exists %s !\n", tx.GetHash().ToString().c_str());
    }
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight, CConnman* connman)
{
    ...
    if (n == -1) {
        LogPrint(BCLog::SWIFTX, "SwiftX::DoConsensusVote - Unknown Masternode\n");
        return;
    }

    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint(BCLog::SWIFTX, "SwiftX::DoConsensusVote - Masternode not in the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint(BCLog::SWIFTX, "SwiftX::DoConsensusVote - In the top %d (%d)\n", SWIFTTX_SIGNATURES_TOTAL, n);
...
bool ProcessConsensusVote(CNode* pnode, CConsensusVote& ctx, CConnman* connman)
{
    ...
    if (pmn != NULL)
        LogPrint(BCLog::SWIFTX, "SwiftX::ProcessConsensusVote - Masternode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);
    ...
    if (n > SWIFTTX_SIGNATURES_TOTAL) {
        LogPrint(BCLog::SWIFTX, "SwiftX::ProcessConsensusVote - Masternode not in the top %d (%d) - %s\n", SWIFTTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }
    ...
    if (!mapTxLocks.count(ctx.txHash)) {
        ...
        LogPrint(BCLog::SWIFTX, "SwiftX::ProcessConsensusVote - Transaction Lock Exists %s !\n", ctx.txHash.ToString().c_str());
    ...
    if (i != mapTxLocks.end()) {
        ...
        LogPrint(BCLog::SWIFTX, "SwiftX::ProcessConsensusVote - Transaction Lock Votes %d - %s !\n", (*i).second.CountSignatures(), ctx.GetHash().ToString().c_str());

        if ((*i).second.CountSignatures() >= SWIFTTX_SIGNATURES_REQUIRED) {
            LogPrint(BCLog::SWIFTX, "SwiftX::ProcessConsensusVote - Transaction Lock Is Complete %s !\n", (*i).second.GetHash().ToString().c_str());

            CTransaction& tx = mapTxLockReq[ctx.txHash];
            if (!CheckForConflictingLocks(tx)) {
#ifdef ENABLE_WALLET
                if (pwallet) {
                        pwallet->UpdatedTransaction((*i).second.txHash);
                        nCompleteTXLocks++;
                }
#endif
