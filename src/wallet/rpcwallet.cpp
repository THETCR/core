...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"unconfirmed_balance\": xxx, (numeric) the total unconfirmed balance of the wallet in WSP\n"
            "  \"immature_balance\": xxxxxx, (numeric) the total immature balance of the wallet in WSP\n"
...
UniValue printMultiSend()
{
    ...
    if (pwalletMain->vDisabledAddresses.size() >= 1) {
        ...
        for (const auto & vDisabledAddresse : pwalletMain->vDisabledAddresses) {
            disAdd.push_back(Pair("Disabled From Sending", vDisabledAddresse));
        }
...
{
    unsigned int sum = 0;
    for (auto & i : pwalletMain->vMultiSend)
        sum += i.second;
    return sum;
}
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zwspTracker->ListMints(true, fMatureOnly, true);
        ...
        for (auto m : setMints) {
            ...
            if (m.hashStake == 0) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zwspTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zwspTracker->ListMints(true, true, true);
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_WALLET_ERROR, "zWSP minting is DISABLED");
    ...
    for (const CDeterministicMint& dMint : vDMints) {
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_WALLET_ERROR, "zWSP minting is DISABLED, cannot mint change");
    ...
    if (!ispublicspend && Params().NetworkID() != CBaseChainParams::REGTEST) {
        throw JSONRPCError(RPC_WALLET_ERROR, "zWSP old spend only available in regtest for tests purposes");
    }

    return DoZwspSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZwspSpend(nAmount, false, true, vMintsSelected, address_str);
}


extern UniValue DoZwspSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
{
    ...
    if(address_str != "") { // Spend to supplied destination address
        ...
        if(!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid WISPR address");
        outputs.emplace_back(std::pair<CBitcoinAddress*, CAmount>(&address, nAmount));
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzWSPTracker* zwspTracker = pwalletMain->zwspTracker.get();
    std::set<CMintMeta> setMints = zwspTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"id\": \"serial hash\",  (string) the mint's zWSP serial hash \n"
    ...
    CzWSPTracker* zwspTracker = pwalletMain->zwspTracker.get();
    std::set<CMintMeta> setMints = zwspTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue searchdzwsp(const UniValue& params, bool fHelp)
{
    ...
    auto* dzwspThreads = new boost::thread_group();
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) WISPR address for auto minting\n" +
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) WISPR address to spend to. If not specified, spend to change add.\n"
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 0)
        throw std::runtime_error(
            "clearspendcache\n"
            "\nClear the pre-computed zWSP spend cache, and database.\n" +
            HelpRequiringPassphrase() + "\n"
