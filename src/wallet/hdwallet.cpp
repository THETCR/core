    ...
    int nChangePosInOut = -1;
    {
        ...
        for (;;) {
            ...
            for (const auto &coin : setCoins) {
                ...
                if (!ProduceSignature(*this, DUMMY_SIGNATURE_CREATOR_WISPR, scriptPubKey, sigdata)) {
...
//        return error("CreateCoinStake : invalid reserve balance amount");
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            mnpayments.FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZWSP());
...
//            const CWalletTx *wtx = GetWalletTx(txIn.prevout.hash);
            if (!SignSignature(keystore, scriptPubKeyKernel, txNew, nIn++, nCredit, int(SIGHASH_ALL|SIGHASH_ANYONECANPAY)))
...
//    CreateCoinStake(pblock->nBits, nSearchTime, nHeight, nFees, txCoinStake, key)
    if (CreateCoinStake(*this, pblock->nBits, nSearchTime - nLastCoinStakeSearchTime, txCoinStake, pblock->nTime)) {
        if (LogAcceptCategory(BCLog::POS)) {
            WalletLogPrintf("%s: Kernel found.\n", __func__);
        }
...
bool IsWisprWallet(const CKeyStore *win)
{
    return win && dynamic_cast<const CHDWallet*>(win);
};

CHDWallet *GetWisprWallet(CKeyStore *win)
{
    CHDWallet *rv;
    if (!win)
...
const CHDWallet *GetWisprWallet(const CKeyStore *win)
{
    const CHDWallet *rv;
    if (!win)
        throw std::runtime_error("Wallet pointer is null.");
...
bool CHDWallet::IsMyZerocoinSpend(const CBigNum& bnSerial) const
{
    return zwspTracker->HasSerial(bnSerial);
}
bool CHDWallet::DatabaseMint(CDeterministicMint& dMint)
{
    CHDWalletDB walletdb(this->GetDBHandle());
    zwspTracker->Add(this, dMint, true);
    return true;
}
bool CHDWallet::GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint)
{
    CMintMeta meta;
...
bool CHDWallet::IsMyMint(const CBigNum& bnValue) const
{
...
bool CHDWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    if (zwspTracker->HasPubcoinHash(hashValue)) {
        ...
        meta.txid = txid;
        return zwspTracker->UpdateState(this, meta);
    } else {
...
bool CHDWallet::GetMint(const uint256& hashSerial, CZerocoinMint& mint)
{
...
bool CHDWallet::SetMintUnspent(const CBigNum& bnSerial)
{
...
//    CHDWalletDB walletdb(this->strWalletFile);
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zwspTracker->UnArchive(this, hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zwspTracker->UpdateZerocoinMint(this, mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zwspTracker->UnArchive(this, dMint.GetPubcoinHash(), true)) {
...
string CHDWallet::ResetMintZerocoin()
{
    long updates = 0;
    long deletions = 0;
//    CHDWalletDB walletdb(this->strWalletFile);
    ...
    for (CMintMeta mint : vMintsMissing) {
        deletions++;
        if (!zwspTracker->Archive(this, mint))
...
bool CHDWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
...
bool CHDWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn,
...
bool CHDWallet::CreateZWSPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
...
bool CHDWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    if (IsLocked()) {
...
string CHDWallet::GetUniqueWalletBackupName(bool fzwspAuto) const
{
...
void CHDWallet::ZWspBackupWallet()
{
...
bool CHDWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    if (!CommitTransaction(wtxNew.tx, wtxNew.mapValue, wtxNew.vOrderForm, reserveKey, g_connman.get(), state)) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zwspTracker->SetPubcoinNotUsed(this, hashPubcoin);
            this->NotifyZerocoinChanged(this, mint.GetValue().GetHex(), "New", CT_UPDATED);
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zwspTracker->SetPubcoinUsed(this, hashPubcoin, txidSpend);
