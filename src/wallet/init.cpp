...
void WalletInit::AddWalletOptions() const
{
    ...
    gArgs.AddArg("-changetype",
                 R"(What type of change to use ("legacy", "p2sh-segwit", or "bech32"). Default is same as -addresstype, except when -addresstype=p2sh-segwit a native segwit output is used when sending to a native segwit address))", false, OptionsCategory::WALLET);
...
bool LoadWallets(interfaces::Chain& chain, const std::vector<std::string>& wallet_files)
{
    for (const std::string& walletFile : wallet_files) {
        ...
        if (fWisprMode && !((CHDWallet*)pwallet.get())->Initialise()) {
            return false;
        }
        AddWallet(pwallet);
    }
