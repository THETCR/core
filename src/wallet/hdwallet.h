...
#ifndef WISPR_WALLET_HDWALLET_H
#define WISPR_WALLET_HDWALLET_H
...
public:
    ...
    CHDWallet(interfaces::Chain& chain, std::string strWalletFileIn) : CWallet(chain, strWalletFileIn) {};
    ...
    bool SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet,
        const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used, AvailableCoinsType nCoinType=ALL_COINS, bool useIX = true) const override EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    bool GetScriptForAddress(CScript &script, const CBitcoinAddress &addr, bool fUpdate = false, std::vector<uint8_t> *vData = nullptr, bool allow_stakeonly = false);
...
//    bool CreateCoinStake(unsigned int nBits, int64_t nTime, int nBlockHeight, int64_t nFees, CMutableTransaction &txNew, CKey &key);
    ...
    {
        zwalletMain = zwallet;
        zwspTracker = std::unique_ptr<CzWSPTracker>(new CzWSPTracker(this, &this->m_location));
    }
