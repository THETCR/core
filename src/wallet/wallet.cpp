...
//                        break;
                ...
                if (coin_control && nFeeNeeded > 0 && CCoinControl->nMinimumTotalFee > nFeeNeeded) {
                    nFeeNeeded = CCoinControl->nMinimumTotalFee;
                }
                if(fUseInstantSend) {
                    nFeeNeeded = std::max(nFeeNeeded, CTxLockRequest(txNew).GetMinFee());
                }
                if (coin_control && CCoinControl->fOverrideFeeRate)
                    nFeeNeeded = CCoinControl->nFeeRate.GetFee(nBytes);
