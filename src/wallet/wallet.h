...
//! -maxtxfee default
//static const CAmount DEFAULT_TRANSACTION_MAXFEE = 1 * COIN; //moved to validation.h
...
{
private:
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl = nullptr, AvailableCoinsType coin_type = AvailableCoinsType::ALL_COINS, bool useIX = true) const;
...
public:
    ...
    bool CreateZerocoinSpendTransaction(CAmount nValue, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, std::string* address = NULL);
    ...
    bool SpendZerocoin(CAmount nValue, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, std::string* addressTo = nullptr);
    ...
    */
    ...
    std::set<std::string> setAutoConvertAddresses;
    ...
    {
        pc_database = WalletDatabase::CreatePrecompute(m_location.GetPath());
        fFileBacked = true;
    }
...
//    bool SignBlock(CBlockTemplate *pblocktemplate, int nHeight, int64_t nSearchTime);
    ...
    std::map<std::string, std::vector<COutput> > AvailableCoinsByAddress(bool fConfirmed = true, CAmount maxCoinValue = 0);
    ...
    std::string GenerateNewAutoMintKey();
    ...
    struct Balance {
        ...
        //!< WISPR
        CAmount m_zerocoin_trusted{0};
        CAmount m_zerocoin_untrusted_pending{0};
        CAmount m_zerocoin_immature{0};
    };
...
//    CAmount GetBalance(const isminefilter& filter=ISMINE_SPENDABLE, const int min_depth=0) const;
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, CAmount> >& vecSend,
                           ...
                           AvailableCoinsType coin_type = AvailableCoinsType::ALL_COINS,
                           bool useIX = false,
                           CAmount nFeePay = 0);
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl = nullptr, AvailableCoinsType coin_type = AvailableCoinsType::ALL_COINS, bool useIX = false, CAmount nFeePay = 0);
    ...
    bool CreateTransaction(interfaces::Chain::Lock& locked_chain, const std::vector<CRecipient>& vecSend, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut,
                           std::string& strFailReason, const CCoinControl& coin_control, bool sign = true, AvailableCoinsType coin_type = AvailableCoinsType::ALL_COINS,
                           bool useIX = false,
                           CAmount nFeePay = 0);
    bool CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, CReserveKey& reservekey, CValidationState& state, std::string strCommand = "tx");
