...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOT10000IFMN, // ONLY_NONDENOMINATED and not 10000 WSP at the same time
    STAKABLE_COINS
};
...
public:
    ...
    virtual bool SelectCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet,
                    const CCoinControl& coin_control, CoinSelectionParams& coin_selection_params, bool& bnb_used, AvailableCoinsType nCoinType=ALL_COINS, bool useIX = true) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    //! For WisprWallet, clear cached balances from wallet called at new block and adding new transaction
