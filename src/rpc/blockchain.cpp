...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zwspObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZWSP()) ? "Not available" : std::to_string(
...
UniValue getmintsinblocks(const UniValue& params, bool fHelp) {
    ...
    validaterange(params, heightStart, heightEnd, Params().NEW_PROTOCOLS_STARTHEIGHT());
